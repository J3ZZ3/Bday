...
// Staggered wave-pop for each letter of "For Sienna"
function AnimatedTitle() {
  const chars = "For Sienna".split("");
  return (
    <h1
      className="font-serif text-5xl sm:text-7xl font-bold text-pink-600 leading-tight"
      ...
      {chars.map((ch, i) => (
        <span
          ...
          style={{
            ...
            animation: "letter-pop 0.6s cubic-bezier(.34,1.56,.64,1) forwards",
...
export default function CountdownPage() {
  ...
  // clear flip flag after animation
  useEffect(() => {
  ...
  return (
    <div className="aurora-bg min-h-screen relative overflow-hidden flex flex-col items-center justify-center px-4">
      ...
      <div
        ...
        style={{ animation: "fade-in-up 0.9s cubic-bezier(.22,1,.36,1) forwards" }}
      >
        {/* Badge */}
        <div
          className="glass-card rounded-full px-4 py-1.5 mb-6 flex items-center gap-2 shadow"
        ...
        <div className="flex items-center gap-3 my-7">
          ...
          <span className="text-pink-500 text-2xl animate-heartbeat inline-block" style={{ filter: "drop-shadow(0 0 8px #ec4899)" }}>♥</span>
        ...
        <div className="grid grid-cols-4 gap-3 sm:gap-5 w-full max-w-md">
          {LABELS.map(({ key, label }, i) => {
            ...
            return (
              ...
              >
                <div
                  className="glass-card rounded-2xl w-full aspect-square flex items-center justify-center relative overflow-hidden shadow-lg animate-pulse-glow"
                ...
                >
                  ...
                  {u.flip && (
                    <span
                      className="absolute timer-digit font-serif text-3xl sm:text-5xl font-bold text-pink-300"
                ...
                <span className="mt-2 text-xs font-semibold tracking-widest uppercase text-pink-400/80">
                  {label}
                </span>
              </div>
            );
        ...
        {/* Sub-message */}
        <div
          className="mt-10 glass-card rounded-2xl px-7 py-4 max-w-sm shadow-lg animate-card-float"
          style={{ animation: "scale-in 0.5s cubic-bezier(.34,1.56,.64,1) 1.1s both, card-float 5s ease-in-out 1.6s infinite" }}
        >
